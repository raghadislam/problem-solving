...
struct LCA {
    ...
    //
    //     int ans = path(u, distU) + path(v, distV); // change the operation according what needed
    //     return ans;
    // }
...
void solve() {
    ...
    while (cin >> s && s!= "DONE") {
        if (s == "DIST") {
            int u, v; cin >> u >> v;
            int lc = lca.lca(u, v);
            cout << lca.path1(u, lca.distance(u, lc)) + lca.path1(v, lca.distance(v, lc))<< "\n";
        }
        else if (s == "KTH") {
            int u, v, k; cin >> u >> v >> k;
            cout << lca.kthNode(u, v, k-1) << "\n";
        }
    }
